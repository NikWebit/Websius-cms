...
class Resources {
    ...
    public static function __init() {
        ...
        if (static::$consolidate) {
            if (!file_exists(cot::$cfg['cache_dir']))
                mkdir(cot::$cfg['cache_dir'], cot::$cfg['dir_perms']);
            if (!file_exists(static::$dir))
                mkdir(static::$dir, cot::$cfg['dir_perms']);
        }
    }
    ...
    public static function minify($code, $type = 'js') {
        if ($type == 'js') {
            ...
            require_once Path::libFile('JShrink');
            $code = \JShrink\Minifier::minify($code);
        } elseif ($type == 'css') {

            require_once Path::libFile('CssMin');
