...
if ($_SERVER['HTTP_HOST'] == $url['host'] || $cfg['multihost'] || $_SERVER['HTTP_HOST'] != 'www.' . $sys['domain'] && preg_match('`^.+\.' . preg_quote($sys['domain']) . '$`i', $_SERVER['HTTP_HOST'])) {
...
if (!empty($csid) || !empty($_SESSION[$sys['site_id']])) {
    ...
    if ($u_id > 0) {
        ...
        if ($row = $sql->fetch()) {
            if ($u_sid == hash_hmac('sha1', $row['user_sid'], $cfg['secret_key']) && ($row['user_maingrp'] > 3 ||
                    ($row['user_maingrp'] == COT_GROUP_INACTIVE && isset($cfg['users']['inactive_login']) &&
                    $cfg['users']['inactive_login'])) && ($cfg['ipcheck'] == FALSE || $row['user_lastip'] == $usr['ip']) && $row['user_sidtime'] + $cfg['cookielifetime'] > $sys['now']) {
...
if ($cfg['shieldenabled'] &&
    ...
    $shield_limit = isset($_SESSION['online_shield']);
    $shield_action = $_SESSION['online_action'] ?? '';
    $shield_hammer = cot_shield_hammer(
            isset($_SESSION['online_hammer']),
            $shield_action,
            $_SESSION['online_lastseen']
    );
...
// Theme resources
$sys['theme_resources'] = defined('COT_ADMIN') && !empty($cfg['admintheme']) ? "{$cfg['themes_dir']}/admin/{$cfg['admintheme']}/{$cfg['admintheme']}.php" : "{$cfg['themes_dir']}/{$usr['theme']}/{$usr['theme']}.php";
